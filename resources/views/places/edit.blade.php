<x-master> 
      <main class="h-full overflow-y-auto">
        ...
        @endif
            ...
            >
              Modifier place/place de parking N°{{$place->num}}
            </h2>
            ...
            <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
              <label class="block text-sm">
                <span class="text-gray-700 dark:text-gray-400">N° de place</span>
                <input
              ...
              <label class="block mt-4 text-sm">
                <span class="text-gray-700 dark:text-gray-400">Surface en m2</span>
                <input
                  class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                  placeholder=""
                  type="number"
              ...
              <label class="block mt-4 text-sm">
                <span class="text-gray-700 dark:text-gray-400">
                  Sur quelle immeuble se trouve ce place/place de parking ?
                </span>
                <select
                ...
                @foreach ($immeubles as $immeuble)
                  <option value="{{ $immeuble->id }}"
                    @if($place->immeuble->id == $immeuble->id)
                      selected
                    @endif                    
              ...
              <label class="block mt-4 text-sm">
                ...
                <input
                  ...
                  step="0.01"
                  value="{{$place->produit->prixM2Indicatif}}"
                  :required="isOpenPrix"
                  :disabled="!isOpenPrix"
              ...
              <label class="block mt-4 text-sm">
                ...
                <input
                  ...
                  value="{{ $place->produit->prixM2Definitif }}"
                  step="0.01"
                  :required="isOpenPrix"
                  :disabled="!isOpenPrix"                    
                />
             ...
             @isset($place->produit->dossier)
              <div
                ...
                @foreach ($place->produit->dossier->clients as $client)
              ...
              <div class="mt-4 text-sm">
                ...
                @foreach ($etiquettes as $etiquette)
                  <option value="{{ $etiquette->id }}"
                    @if($place->produit->etiquette_id == $etiquette->id)
                      selected
                    @endif
                    >
                    {{$etiquette->label}}
