<x-master>
      <main class="h-full overflow-y-auto">
        ...
        @endif
            ...
            >
              Enregistrer le désistement de vente {{$dossier->produit->constructible_type}} N° {{$dossier->produit->constructible->num}}
            </h2>
...
<form action="/dossiers/{{ $dossier->id }}/demandeDesisstement" method="POST">
    @csrf

              <label class="block mb-4 text-sm">
                <span class="text-gray-700 dark:text-gray-400">Motif désistement</span>
                <textarea
                  class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-textarea focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray"
                  rows="3"
                  placeholder="Motif désistement"
                  name="motif"
                  required

                >{{$dossier->motif_desistement}}</textarea>
                    @error('motif')
              ...
              <label class="block text-sm mt-4 mb-4">
                <span class="text-gray-700 dark:text-gray-400">Taux indémnité à déduire en pourcentage :</span>
                <input
              ...
              <div class="flex">
                ...
                <button
                  ...
                  type="submit"
                >
                    Générer la demande de désistement
                </button>
              </div>
            ...
            <div
              class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800"
            >            
            <form action="/dossiers/{{$dossier->id}}/desister" method="POST" enctype="multipart/form-data">
              @csrf
              @method('PUT')

<!-- début upload pièce jointe -->
              <div class="mt-4 text-sm" x-show="isOpen">
                    @error('demandeDesisstement')
                    <p class="block h-10 px-2 py-2 rounded-md w-full mt-2
                    bg-red-600 text-white font-bold"> Attention : {{ $message }}</p>
                    @enderror

                <span class="text-gray-700 dark:text-gray-400">
                  Uploader la demande de désistement légalisée.
                </span>
                <div class="mt-2">
    ...
    <input
    type="file"
    name="demandeDesisstement"
    id="demandeDesisstement"
    required
    >
